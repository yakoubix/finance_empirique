...
plot(rcor,main = "The correlation between banking sector actions",xlab = "Date",cex.axis = 2)
=======
```
>>>>>>> 554647608414fb2eb4eb7ca521897c4a4bbbb606

```{r, fig-5, echo=FALSE, fig.width=15, fig.cap="zooplot"}
index(rcor) = as.Date(date(index(rcor)), "%Y-%m-%d")
plot(rcor,main = "The correlation between banking sector actions",xlab = "Date",cex.axis = 2)
```
```{r}
colMeans(rcor)
```
Les 3 entreprises choisies sont du même secteur d'activités. Leur corrélation dans le temps est comprise entre 0.4 et 0.9 (avec un pic à -0.2) avec une moyenne d'environ 0.75. Elle est donc forte.Nous observons une même tendance d'un graphe à l'autre. En effet, lorsque la corrélation augemente entre CACIB et Société Générale CIB, elle augmente également entre BNP CIB et Société générale, et inversement. 


```{r, fig-5, warning=FALSE, echo=FALSE, fig.width=20, fig.cap="Correl"}

CAClev <- as.zoo(ts.all)[, c("ker.pa", "bn.pa", "ora.pa")]
rcor <- rollapply(CAClev, width = 10, rollc, align = "right",by.column = FALSE)
colnames(rcor) <- c("KERING & DANONE", "KERING & ORANGE", "ORANGE & DANONE") 
index(rcor) = as.Date(date(index(rcor)), "%Y-%m-%d")
plot(rcor,main = "The correlation between banking sector actions",xlab = "Date",cex.axis = 2)
```
...
```{r, fig-6, warning = FALSE, echo=FALSE, fig.width=15, fig.cap="COV"}
...
```{r, echo=TRUE}
print(res.pca.1$x[,c('PC1','PC2')])
```
Nous remarquons que les deux premières composantes contiennent plus de 96% de l'information notamment 78% de la variance est concentrée dans la première composante. Ainsi la première composante représente une forte partie du CAC40. 
